...
Y <- Mu1*ifelse(x<0, 1, 0) + Mu2*ifelse(x>0, 1, 0) + Sig*Ep
...
## HRDD
HRDD <- HRDD_cont(Y=Y, x=x, ID=ID, band="local", h=hg, c=0, order=1)
Est[,3] <- apply(HRDD$Tau, 2, mean)
CI4 <- t( apply(HRDD$Tau, 2, quantile, prob=c(0.025, 0.975)) )


## RMSE
sqrt( apply((Est-Tau.true)^2, 2, mean) )


## Figure 
bb <- 0.2
ind <- order(Tau.true)
plot(Tau.true[ind], ylim=range(CI3), ylab="Treatment effect", xlab="Group", pch=20, col=1)
